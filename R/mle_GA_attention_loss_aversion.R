...
mle_GA_attention_loss_aversion <- function(data, popSize, generations, num_cores, lower = c(0.00001, 0.01, 0, 0), upper = c(0.01, 0.1, 2, 1),
  ...
  maximum_likelihood_genetic_algorithm_C <- function(data, d, sigma, lambda, theta, timeStep, barrier){
    ...
    get_trial_likelihood_C <- function(value_up_boundary, value_down_boundary, d, lambda, sigma, theta, timeStep, 
    ...
    #Save each model
    fitness <- data.frame( d=d, sigma=sigma, lambda=lambda, theta=theta, NLL = nll )
