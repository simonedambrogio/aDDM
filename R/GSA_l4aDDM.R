####################################### GSA_l2a #######################################
GSA_l4aDDM <- function(data, d_set, sigma_set, lambda_set, theta_gain_set, theta_loss_set, timeStep = 10, barrier = 1, numCores){
  ...
  #mle_ala function
  mle_ala <- function (data, d, sigma, lambda, theta_gain, theta_loss, timeStep = 10, barrier = 1){
    
    get_trial_likelihood_C <- function(value_up_boundary, value_down_boundary, 
                                       d, theta_gain, theta_loss, lambda, sigma, timeStep, approxStateStep = 0.1, 
      ...
      media <- sapply(seq_along(FixItem), function(i) {
        if (FixItem[i] == 1) {
          mean <- d * (value_up_boundary - (theta_gain * (lambda * value_down_boundary)))
        }
        else if (FixItem[i] == -1) {
          mean <- d * ((theta_loss * value_up_boundary) - (lambda * value_down_boundary))
        }
        else if (FixItem[i] == 3) {
          mean <- 0
        }
    ...
    likelihood <- unlist(foreach(trial_i = unique(data$trial)) %dopar% get_trial_likelihood_C(value_up_boundary = unique(data[data$trial == trial_i, "value_up_boundary"]), 
                                                                                              ...
                                                                                              d = d, lambda = lambda, sigma = sigma, theta_gain = theta_gain, theta_loss = theta_loss,
    ...
    print(paste0("Calcolo NLL modello: d = ", d, " sigma = ", sigma, " lambda = ", lambda, " theta_gain = ", theta_gain, " theta_loss = ", theta_loss, " --- NLL = ", round(nll) ))
    return(nll)
  }
  ...
  while (improvement >= 0.01){
    ...
    par_set <- expand.grid(d=d_set, sigma=sigma_set, lambda=lambda_set, theta_gain=theta_gain_set, theta_loss=theta_loss_set)
    ...
    grid_results <- sapply( 1:nrow(par_set), function(row_i){
      mle_ala(data=data, d=par_set[row_i, 'd'], sigma=par_set[row_i, 'sigma'], 
              lambda=par_set[row_i, 'lambda'], theta_gain=par_set[row_i, 'theta_gain'],
              theta_loss = par_set[row_i, 'theta_loss'], timeStep = timeStep, barrier = barrier)
    })
